...
if [[ -z ${VERSION} ]]; then
  ...
  VERSION="$(curl -s https://api.github.com/repos/enzyme-health/opta/releases/latest | grep 'tag_name' | grep -oP '[0-9.]+')"
else
...
if [[ ${OS} == "Linux" ]]; then
  ...
  if [[ ${SPECIFIC_OS_ID} == "amzn" ]] || [[ ${SPECIFIC_OS_ID} == "centos" ]]; then
    PACKAGE=https://github.com/enzyme-health/opta/releases/download/v$VERSION/opta_centos.zip
  else
    PACKAGE=https://github.com/enzyme-health/opta/releases/download/v$VERSION/opta_linux.zip
  fi
elif [[ ${OS} == "Darwin" ]]; then
  PACKAGE=https://github.com/enzyme-health/opta/releases/download/v$VERSION/opta_mac.zip
else
...
if [[ $? != 0 ]]; then
  ...
  echo "Please check the available versions at https://github.com/enzyme-health/opta/releases."
  exit 1
fi
echo "Downloaded"
