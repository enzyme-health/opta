  ...
  values = [
    yamlencode({
      ...
      containerResourceRequests = {
        cpu    = var.resource_request["cpu"],
        memory = var.resource_request["memory"]
      },
      deployPods = (local.uppercase_image != "AUTO") || (var.tag != null) || (var.digest != null),
      image = local.image,
      version = var.tag == null ? "latest" : var.tag,
      livenessProbePath = coalesce(var.liveness_probe_path, var.healthcheck_path),
      livenessProbeCommand = coalesce(var.liveness_probe_command, var.healthcheck_command),
      initialLivenessDelay = var.initial_liveness_delay,
      readinessProbePath = coalesce(var.readiness_probe_path, var.healthcheck_path),
      readinessProbeCommand = coalesce(var.readiness_probe_command, var.healthcheck_command),
